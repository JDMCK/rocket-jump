...
@export var data: RocketLauncherData
...
func _ready() -> void:
	assert(data != null, "Rocket launcher must have a data resource")
	# Set attributes of cooldown timer
	cooldown.wait_time = data.cooldown_sec
...
func fire(direction: Vector2) -> void:
	...
	rocket_scene.velocity = direction * data.rocket_speed
